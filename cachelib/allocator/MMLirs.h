...
// in bytes. See MMLirs-inl.h::maybeGrowAccessCountersLocked()
  ...
  struct Container {
   ...
   public:
    ...
    class Iterator {
     ...
     private:
      // private because it's easy to misuse and cause deadlock for MMLirs
   ...
   private:
    ...
    void shiftHashListToNext(T& node, uint64_t nodeHashVal) noexcept;

    // update rmax if necessary
    void maybeMoveRmaxLocked() noexcept;

    void promoteHirToMainTop(T& node,
                             uint64_t nodeHashVal,
                             bool isMoving) noexcept;

    void moveHirToTinyTop(T& node,
                          uint64_t nodeHashVal,
                          bool isMoving) noexcept;

    void moveLirToMainTop(T& node,
                          uint64_t nodeHashVal,
                          bool isMoving) noexcept;

    void accessInLirs(T& node, bool isMoving) noexcept;
    ...
    std::atomic<uint32_t> underRmax_{0};
    ...
    // Config for this lru.
    // Write access to the MMLirs Config is serialized.
    // Reads may be racy.
    Config config_{};

    static constexpr uint64_t kLirPtrTombstone{0};

    std::unordered_map<uint64_t, T*> mapHirHash2LirPtr_;
    std::unordered_map<uint64_t, std::unordered_set<uint64_t>>
        mapLirHash2HirHash_;

    FRIEND_TEST(MMLirsTest, SegmentStress);
    FRIEND_TEST(MMLirsTest, LirsBasic);
    FRIEND_TEST(MMLirsTest, Reconfigure);
  };
};
} // namespace cachelib
} // namespace facebook

#include "cachelib/allocator/MMLirs-inl.h"
