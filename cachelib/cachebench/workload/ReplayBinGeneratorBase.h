...
class ReplayBinGeneratorBase : public GeneratorBase {
 public:
  explicit ReplayBinGeneratorBase(const StressorConfig& config)
    ...
    infile_.open(file, std::ios::binary);
    if (infile_.fail()) {
  ...
  virtual ~ReplayBinGeneratorBase() { infile_.close(); }
